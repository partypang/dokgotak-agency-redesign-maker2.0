...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = [
      `독고탁 에이젼시 ${EDIT_ENGINE_VERSION} 섹션 편집 엔진이다.`,
      "첨부된 9:16 상세페이지 섹션 이미지를 기반으로 같은 제품과 전체 톤앤매너를 유지하면서 필요한 부분만 편집한다.",
      "2.0 편집 원칙: 사용자가 요청한 부분만 바꾸고, 제품 실물/브랜드/수치/인증/혜택 조건은 원본과 다르게 만들지 않는다.",
      ...
      "출력 품질: 이전 섹션과 비교했을 때 여백, 헤드라인 위치, 제품컷 크기, 정보 카드 모양 중 최소 2가지는 달라야 한다. 단, 브랜드 톤과 핵심 제품 인식은 유지한다."
    ].join("\n");
