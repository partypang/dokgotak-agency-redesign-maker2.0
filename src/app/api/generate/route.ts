...
const GENERATION_ENGINE_VERSION = "2.0";
...
async function analyzeSource({
  ...
  const prompt = [
    ...
    "JSON 키: product_inferred, verified_assets, conversion_risks, diagnostic_summary, strategy, section_message_map, page_blueprint, compliance_notes"
  ].join("\n");
...
function buildSections(
  ...
  return sectionTemplates(count, startSection).map((template) => {
    const promptText = [
      `독고탁 에이젼시 ${GENERATION_ENGINE_VERSION} 이미지 생성 엔진이다.`,
      `이미지 생성 모델: ${modelInfo.label} (${modelInfo.id})`,
      ...
      `분석 요약: ${JSON.stringify(analysis).slice(0, 3200)}`,
      "2.0 핵심 규칙: 원본에서 확인된 제품 사진, 패키지, 색감, 구성품, 가격/혜택/인증/리뷰 단서는 보존하고, 확인되지 않은 새 주장은 추가하지 않는다.",
      "서비스명 금지 규칙: '독고탁', '독고탁 에이젼시', '독고탁에이젼시', 'Dokgotak', 'DOKGOTAK', 'DGT'는 이 도구의 서비스명일 뿐이며 제품 브랜드가 아니다. 이 단어들을 이미지 안의 제품명, 브랜드명, 로고, 라벨, 헤드라인, 후기, FAQ, CTA, 패키지 텍스트로 절대 사용하지 않는다.",
      "브랜드 사용 규칙: 제품 브랜드명과 제품명은 업로드된 원본 상세페이지 또는 제품 패키지에서 확인되는 이름만 사용한다. 원본에서 확인되지 않는 새 브랜드명, 새 제품명, 새 로고를 만들지 않는다.",
      "텍스트 보존 규칙: 원본에 있는 핵심 수치, 용량, 구성, 가격, 배송/혜택 조건은 왜곡하지 않는다. 읽기 어려운 작은 글씨를 새로 많이 만들지 않는다.",
...
function channelPlaybook(channel: string) {
  if (channel.includes("쿠팡")) {
    return "가격/구성/배송/후기 신뢰를 먼저 보여주고, 모바일에서 1초 안에 읽히는 굵은 카피와 정보 카드 중심으로 구성한다.";
  }
  if (channel.includes("자사몰")) {
    return "브랜드 무드와 제품 차별성을 함께 보여주고, 보증/후기/스토리로 신뢰를 쌓은 뒤 CTA로 연결한다.";
  }
  if (channel.includes("와디즈")) {
    return "문제 제기, 해결책, 제작 이유, 검증, 리워드/FAQ 흐름을 서사적으로 연결한다.";
  }
  return "스마트스토어 비교 구매 맥락에 맞춰 상품명, USP, 혜택, 리뷰/인증, 구매 불안 해소를 빠르게 스캔되도록 구성한다.";
}
